...
- [راه‌اندازی Memcached در سمت سرور](#راه-‌اندازی-Memcached-در-سمت-سرور)
- [راه‌اندازی Memcached در سمت کلاینت](#راه-‌اندازی-Memcached-در-سمت-کلاینت)
- [منابع](#منابع)
...
Memcached یک سیستم ذخیره‌سازی و کش کردن دیتا به صورت توزیع شده می‌باشد. با استفاده از Memecached می‌توانیم با ذخیره‌سازی
داده‌ها در RAM، تعداد درخواست برای خواندن دیتا از پایگاه‌داده را کاهش دهیم.
...
همچنین سیستم طوری طراحی شده‌ است که از طریق TCP و UDP قابل دسترس باشد. در نتیجه سرور‌ها می‌توانند ساختار توزیع شده (
distributed) و یا مجزا (separated) داشته باشند.
اگر حافظه‌کش پر شود، سیستم به طور اتوماتیک داده جدید را جایگزین قدیمی‌ترین داده می‌کند در نتیجه باید از آن به عنوان یک
...
2. اگر داده مدنظر در کش موجود بود، آن داده را برگردانده می‌شود. در غیر این صورت داده مدنظر در دیتابیس مورد جست‌وجو قرار
   می‌گیرد.
3. اگر زمان منقضی شدن یک داده گذشته باشد، کش آپدیت می‌شود تا از تازه بودن داده‌های در دسترس کاربر اطمینان حاصل شود.

# راه‌اندازی Memcached در سمت سرور

در ادامه می‌خوایم یک حافظه کش را با کمک Memcached ایجاد کنیم. دقت شود که تمامی دستورها استفاده شده بر روی سیستم لینوکس
قابل اجرا می‌باشد.
...
پس از نصب، یک فال کانفیگ ایجاد می‌شود که می‌توان با تغییر آن نحوه اجرا شدن Memcached را کنترل کرد. در ادامه به معرفی
...
میزان حافظه‌ای که از RAM به کش اختصاص داده می‌شود. در صورتی که حافظه اختصاص داده شد از حافظه موجود در RAM بیشتر باشد، از swap space استفاده می‌شود که سرعت پاسخ را به میزان زیادی کاهش می‌دهد.
...
حداکثر تعداد کانکشن‌هایی که می‌تواند همزمان رخ دهد. مقدار اولیه‌ آن برابر ۱۰۲۴ می‌باشد.
...
تعداد تر‌دهای مورد استفاده برای اجرا کردن درخواست‌ها. مقدار اولیه آن برابر ۴ می‌باشد.

برای آشنایی بیشتر با فلگ‌های موجود در کانفیگ می‌توانید به رفرنس ۳ مراجعه کنید.
...
برای آنکه بتوانیم تحت ترمینال با Memcached ارتباط برقرار کنیم نیاز به کتابخانه libmemcached-tools داریم که با استفاده از
