...
در مثال زیر همه موارد جدید ایجاد شده به طور خودکار ویژگی‌های سبک انحنای فعلی را همان‌طور که در رابط Illustrator تعریف شده است دریافت می کنند. همچنین می‌توانیم با استفاده از این کدها را از طریق تابع currentStyle تغییر دهیم.
...
در قطعه کد زیر 150 دایره ایجاد شده و با استفاده از symbol در جایی از صفحه قرار می گیرد.(ابتدا دایره به سیمبل bind می‌شود و با دستور symbol.place(center) یک دایره به symbol اضافه می‌شود) سپس در onFrame handler در یک لوپ دایره ها از طریق لایه های اکتیو project گرفته می‌شود (متغیرهای موجود در symbol) و مولفه x آن‌ها در صفحه به اندازه 1/20 عرض هرکدام افزایش پیدا می‌کند و پس از طی کردن صفحه دوباره از سمت چپ سروع به حرکت می کنند. 
