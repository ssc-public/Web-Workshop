...
نمونه قابل دنبال کردن این پوشه در مسیر `unit-vs-int` قرار دارد. این پروژه یک کنترلر کوچک به نام `HelloController` دارد که مسیر `/hello` را پیاده‌سازی می‌کند. تست‌های همین نمونه در `HelloControllerTest.java` و `HelloControllerTest_Integration.java` نوشته شده‌اند و با JUnit Jupiter و MockMvc کار می‌کنند.

تصاویر مربوط به `UserService` و `UserController` در ادامه، مثال‌های مفهومی برای توضیح نوع تست هستند و بخشی از پروژه `unit-vs-int` نیستند.
