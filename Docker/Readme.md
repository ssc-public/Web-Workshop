...
از طرفی هزاران برنامه و نرم‌افزار متنوع وجود دارند که برای کار کردن در محیط ```Docker``` طراحی شده اند و شما به عنوان یک ITPRO می‌توانید به راحتی از آن‌ها در مجموعه خودتان در قالب یک Docker Container استفاده کنید. از طرفی در محیط های عملیاتی```Docker``` این امکان را به همه می‌دهد که چندین برنامه را همزمان بر روی یک سیستم فیزیکی نصب و اجرا کنند و این‌ها هیچکدام با یکدیگر کوچکترین ارتباطی نداشته باشند و به صورت کاملا ایزوله در مجموعه فعالیت کنند.
## کانتینرها و داکر
