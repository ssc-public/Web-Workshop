...
### Enumeration types
<p dir="rtl" style="position:right;">
enum ها نوعی از scalar ها هستند که فقط توانایی ذخیره چند مقدار محدود و از پیش تعیین شده را در خود دارند و در هر لحظه فقط می‌توانند یکی از مقادیرشان را اتخاذ کنند. کاربرد enum  در graphql بسیار شبیه به سایر زبان های برنامه نویسی است فقط با این تفاوت که qraphql فیلد های enum را به integer تبدیل نمیکند و هنگام ارسال query تنها ماهیت رشته ای فیلد های آن منتقل میشود.<br>
...
```
<p dir="rtl" style="position:right;">
حال ۲ تایپ Circle و Triangle را به شکل زیر تعریف می‌کنیم که تایپ Shape را implement میکنند.
...
```
<p dir="rtl" style="position:right;">
در مثال بالا تمامی فیلد هایی که در تایپ Shape قرار گرفته اند, در تمامی تایپ هایی که آن را implement میکنند آمده است. هر آبجکت از تایپ Shape فقط می‌تواند Circle باشد یا اینکه Triangle باشد. تایپ های Circle و Triangle دارای فیلد هایی هستند که در تایپ Shape نیست و به وسیله همان ها از سایر تایپ ها متمایز میشوند.<br>
