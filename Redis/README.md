...
ردیس، مجموعه ای از امکانات داخلی از قبیل تکرار (replication)، اسکریپت‌نویسی لوآ (Lua scripting)، تخلیه LRU و ... را ارائه میدهد.
...
در نسخه‌های اخیر بهینه‌سازی‌هایی انجام شده است که نتایج به دست آمده را به نتیجه حاصل از اجرای دقیق این الگوریتم نزدیک‌تر می‌کند.
...
دلیل استفاده نشدن از نسخه دقیق این الگوریتم این است که موجب استفاده از مموری بیشتری می‌شود. هر چند که نتایج تا حدی خوبی نزدیک به هم است. لینک زیر توضیح اصلی درباره این مقایسه را نشان می‌دهد.

[توضیح اصلی LRU در Redis](https://antirez.com/news/109)
...
`ZCOUNT` تعداد اعضای مجموعه را بر می‌گرداند که امتیاز آنها در بازآ مینیمم تا ماکسیمم داده شده باشد.
