...
listen
می‌کند و در پاسخ درخواست‌های get
...
هندلرها یک ورودی از جنس پوینتر به gin.Context می‌گیرند؛ این ورودی شامل اطلاعات مربوط به درخواست شامل
...
انواع مختلفی از پارامترها در هر درخواست HTTP
...
برای خواندن مقدار Headerهای HTTP از درخواست می‌توانیم از متد `GetHeader` تایپ `gin.Context` استفاده کنیم.
...
هم‌چنین gin به ما امکان خواندن فیلدهای گوناگون از body درخواست به فرمت‌های گوناگون JSON و XML و ... را می‌دهد.
...
`FromFile`
استفاده می‌کنیم تا بتوانیم فایل را از درخواست پارس کنیم. سپس چک می‌کنیم که آیا مشکلی در گرفتن فایل وجود دارد یا خیر
...
در زیر یک نمونه ساده را نشان می‌دهیم که مانند مثال‌های قبلی اطلاعات کاربر را از body درخواست او می‌خواند و آن‌ها را در یک فایل HTML به او نشان می‌دهد. در این مثال ما از پکیج استاندارد golang استفاده می‌کنیم.
...
حالا باید از کد golang خود این فایل template‍ را صدا بزنیم و با داده‌های کاربر پر کنیم. در ابتدا لازم است با فراخوانی متد `LoadHTMLGlob` این templateها را در روتر خود لود کنیم. حالا مانند مثال قبل اطلاعات کاربر را از درخواست می‌خوانیم و در متغیر `user` نگه می‌داریم. در نهایت کافی‌ست با متد `HTML` عملیات rendering را انجام دهیم.
