...
برای ایجاد کلاینت و سرور به دست آورده شده‌اند.
...
برای پیاده‌سازی سرور از کتابخانه `ws` استفاده می‌کنیم.
...
کتابخانه `ws` اصطلاحا event-driven است و باید برای eventهای مختلف handler مشخص کنیم.
...
به‌جای کتابخانه‌های third-party از `WebSocket` API متعلق‌به W3C استفاده می‌کنیم.
